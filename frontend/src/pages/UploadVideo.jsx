      ...
      <form onSubmit={submit} className="panel">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-slate-500">MP4, MOV, AVI, MKV, or WEBM with a visible full-body player works best.</p>
          </div>
          <ModeToggle mode={mode} setMode={setMode} />
        </div>
        <label className="grid min-h-72 cursor-pointer place-items-center rounded-xl border-2 border-dashed border-line bg-canvas p-8 text-center transition hover:border-pitch hover:bg-pitch/5">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-ink">{file ? file.name : "Select cricket clip"}</p>
            <p className="mt-2 text-slate-500">The backend samples frames, detects landmarks, aligns sequences, and ranks references.</p>
          </div>
        </label>
